...
This page reports how thoroughly the analysis pipeline captured the repository's structure. The pipeline processed 21 files and identified 6 distinct modules, generating 81 embeddings that power semantic search and navigation across the documentation. No routes were detected, which suggests this codebase may be a library, CLI tool, or service that doesn't expose HTTP endpoints.

The analysis completed without degradation—meaning the pipeline encountered no missing dependencies, parsing failures, or other issues that would prevent full extraction. All targeted files were successfully processed, giving you complete visibility into the modules and entities the system detected.

Use the tables below to verify coverage across the repository's key areas. If expected modules or files are missing, check that they match the pipeline's language and framework filters.
<!-- /alpha8-narrative:overview -->
