# Geospatial indexing and visualization (quadtree-based nearest-neighbor search) Documentation — Quality and coverage

<!-- alpha8-narrative:overview -->
## Overview

This page reports on the documentation coverage achieved by the analysis pipeline across the repository. The pipeline processed 21 files and generated 81 embeddings, capturing code structure across 6 modules. No routes were detected, indicating this codebase may focus on library components, utilities, or non-web application logic rather than HTTP endpoints.

The analysis completed without degradation, meaning the pipeline successfully parsed and indexed all targeted files without encountering blocking errors or timeouts. No entities were extracted, which may reflect the repository's architectural style or the absence of certain patterns like ORM models or data transfer objects that the pipeline typically recognizes.

The tables below break down which files contributed to the embedding count and which modules were identified, giving you visibility into what the pipeline saw and where coverage exists. Use this information to verify that critical paths in your codebase are represented in the generated documentation.
<!-- /alpha8-narrative:overview -->
