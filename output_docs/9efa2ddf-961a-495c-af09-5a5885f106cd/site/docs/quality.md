# Geospatial indexing and visualization — quadtree-based proximity search over latitude/longitude points Documentation — Quality and coverage

<!-- alpha8-narrative:overview -->
## Overview

This page reports on the documentation coverage achieved by the analysis pipeline across the repository. The pipeline successfully processed 21 files and generated 81 embeddings, capturing structural information organized into 6 modules. No degradation occurred during analysis, meaning all targeted files were parsed and indexed without errors.

The absence of detected routes reflects either a codebase without web endpoints or a repository structure the pipeline doesn't currently instrument for route extraction. The zero entity count indicates no explicit domain models or data structures were catalogued, though this varies by project type and the pipeline's configured extractors. The embedding count of 81 suggests moderate documentation density — enough semantic fragments to support search and context retrieval for this project's scale.

Review the tables below to see which files contributed to coverage and whether any critical paths were omitted. If key modules appear missing, consider whether they match the pipeline's current file type and pattern support.
<!-- /alpha8-narrative:overview -->
